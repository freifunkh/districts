...
def is_point_in_polygon(px, py, polygon):
    ...
    4. The number of intersections is infinit => point is exactly on a border of the polygon. We better check that
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='A script to add districts to a Freifunk nodes.json. \
        Needs a GeoJSON file to get the data from.')
    ...
    parser.add_argument('-x', '--output-outsiders-json', help='Output outsiders.json file, showing only nodes who are \
        in no given district.')
    parser.add_argument('-w', '--whitelist-file', help='Use a whitelist for the districts (one name per line). If the \
        district name is not in the list, the default name is used.')
    parser.add_argument('-s', '--sanitize-districts', help='Remove some special characters from district names and \
        convert them to lower case.', action="store_true")
    ...
    for node in nodes_json['nodes']:
        if 'location' not in node['nodeinfo']:
        ...
        if whitelist and district not in whitelist:
            district = args.default_district
